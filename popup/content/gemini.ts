...
function injectBubbleSaveIcons() {
  // Find all user message text lines in Gemini - they use p.query-text-line
  const queryTextElements = document.querySelectorAll('p.query-text-line');

  queryTextElements.forEach((textEl) => {
    const parent = textEl.parentElement as HTMLElement | null;
    if (!parent) return;

    // Skip if already processed for this prompt container
    if (processedUserBubbles.has(parent)) return;

    const lines = Array.from(parent.querySelectorAll('p.query-text-line'))
      .map((line) => line.textContent?.trim() || "")
      .filter((line) => line.length > 0);
    const promptText = lines.join("\n").trim();
    if (!promptText || promptText.length < 2) return;

    // Mark the whole container as processed
    processedUserBubbles.add(parent);

    // Check if we already added an icon in the parent
    if (parent.querySelector('.pp-bubble-save-icon')) return;

    // Create and insert the save icon
    const saveIcon = createBubbleSaveIcon(promptText);

    // Add the icon to the parent container
