...
// Filter prompts by search query (case-insensitive)
function filterPromptsBySearch(items: PromptItem[], query: string): PromptItem[] {
...
function setupEventDelegation() {
  ...
  app.addEventListener("input", (e) => {
    ...
    // Handle search input
    if (target.id === "pp-search" && target instanceof HTMLInputElement) {
      ...
      searchDebounceTimer = setTimeout(async () => {
        ...
        const searchInput = document.getElementById("pp-search") as HTMLInputElement | null;
        if (searchInput) {
          searchInput.focus();
...
async function render(forceRefresh = false) {
  ...
  // Apply search filter
  const searchFilteredItems = filterPromptsBySearch(filteredItems, searchQuery);
  const allGroups = groupPrompts(searchFilteredItems);
  ...
  app.innerHTML = `
    <div class="pp-wrap">
      ...
      <input type="text" class="pp-search" id="pp-search" placeholder="Search prompts..." value="${esc(searchQuery)}" />

      ${groups
        .map((group) => {
          // Auto-expand if: active source matches, OR search query matches prompts in this group
          const hasSearchMatch = searchQuery.trim() && group.prompts.length > 0;
          const isActiveSource = activeSource && group.source === activeSource && group.type === "source";
          const openAttr = isActiveSource || hasSearchMatch ? "open" : "";
