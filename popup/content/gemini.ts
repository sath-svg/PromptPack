...
function injectBubbleSaveIcons() {
  // Find all user message text elements in Gemini - they use p.query-text-line
  const queryTextElements = document.querySelectorAll('p.query-text-line');

  queryTextElements.forEach((textEl) => {
    // Skip if already processed
    if (processedUserBubbles.has(textEl)) return;
    ...
    // Mark as processed
    processedUserBubbles.add(textEl);

    // Check if we already added an icon in the parent
    if (textEl.parentElement?.querySelector('.pp-bubble-save-icon')) return;
    ...
    if (textEl.parentElement) {
      const parent = textEl.parentElement as HTMLElement;
