 ...
 * Styled to match Grok's icon buttons
 */
function createBubbleSaveIcon(promptText: string): HTMLButtonElement {
  const btn = document.createElement("button");
  btn.className = "pp-bubble-save-icon inline-flex items-center justify-center";
  btn.type = "button";
  ...
  btn.innerHTML = `<img src="${iconUrl}" width="16" height="16" alt="Save" style="opacity: 0.8;">`;
...
/**
 * Find and inject save icons next to user message bubbles in Grok
 * Places icon to the LEFT of the Edit button
 */
function injectBubbleSaveIcons() {
  // Find all Edit buttons with aria-label="Edit"
  ...
  editButtons.forEach((editBtn) => {
    ...
    // Find the user message text - traverse up to find the message container
    let msgContainer: HTMLElement | null = editBtn as HTMLElement;
    let promptText = "";

    for (let i = 0; i < 15 && msgContainer; i++) {
      // Look for the message text in various possible containers
      const textEl = msgContainer.querySelector('[class*="whitespace-pre-wrap"]') ||
                     msgContainer.querySelector('[class*="break-word"]');
      if (textEl && textEl.textContent) {
        promptText = textEl.textContent.trim();
        break;
      }
    ...
    processedUserBubbles.add(buttonContainer);

    // Create and insert the save icon BEFORE the edit button (to the left)
    const saveIcon = createBubbleSaveIcon(promptText);
    buttonContainer.insertBefore(saveIcon, editBtn);

    // Find the hoverable group container
    let hoverTarget: HTMLElement | null = editBtn as HTMLElement;
    for (let i = 0; i < 10 && hoverTarget; i++) {
      if (hoverTarget.classList.contains('group') ||
          hoverTarget.getAttribute('class')?.includes('group')) {
        break;
      }
    ...
    if (hoverTarget) {
      // Show icon on hover (matches the group hover behavior)
