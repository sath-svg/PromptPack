...
// Content script for DeepSeek - Pro users only
...
/**
 * Find and inject save buttons next to user message bubbles in DeepSeek
 * Places button to the RIGHT of the user input bubble
 */
function injectBubbleSaveIcons() {
  // DeepSeek user messages have structure:
  // <div class="d29f3d7d ds-message _63c77b1"><div class="fbb737a4">user text</div></div>
  // AI responses have .ds-markdown class inside them

  // Find all ds-message elements that have the fbb737a4 text div (user messages)
  const userMessages = document.querySelectorAll('.ds-message');

  userMessages.forEach((msgEl) => {
    if (!(msgEl instanceof HTMLElement)) return;

    // Skip if already processed
    if (msgEl.dataset.ppProcessed === "true") return;

    // Skip AI responses - they have .ds-markdown inside
    if (msgEl.querySelector('.ds-markdown')) return;

    // Find the text div - user messages have class fbb737a4
    const textDiv = msgEl.querySelector('div[class*="fbb737a4"]');
    if (!textDiv) return;

    const promptText = textDiv.textContent?.trim();
    if (!promptText || promptText.length < 1) return;

    // Mark as processed
    msgEl.dataset.ppProcessed = "true";

    // Make the message element position:relative
    const msgStyle = window.getComputedStyle(msgEl);
    if (msgStyle.position === "static") {
      msgEl.style.position = "relative";
    }

    // Create save button with PromptPack logo - position absolutely to the right
    const saveBtn = document.createElement("button");
    ...
    // Insert save button inside the message element
    msgEl.appendChild(saveBtn);

    processedUserBubbles.add(msgEl);

    // Show/hide on hover of the message
...
async function showContextMenu(x: number, y: number) {
  ...
  const groups = await listPromptsBySourceGrouped("deepseek");
