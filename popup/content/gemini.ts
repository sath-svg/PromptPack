...
const ENHANCE_OFFSET_RIGHT = 150;
const ENHANCE_OFFSET_BOTTOM = 90;
...
function applyEnhanceTheme(theme: ThemeMode) {
  ...
  if (enhanceBtn) {
    ...
    enhanceBtn.style.borderRadius = "8px";
  }
...
function showEnhancePreview(original: string, enhanced: string) {
  ...
  const panelBg = currentTheme === "dark" ? "rgba(15, 15, 15, 0.98)" : "#ffffff";
  const panelText = currentTheme === "dark" ? "#f9fafb" : "#111827";
  const panelBorder = currentTheme === "dark" ? "rgba(255, 255, 255, 0.12)" : "rgba(0, 0, 0, 0.12)";
  const fieldBg = currentTheme === "dark" ? "rgba(20, 20, 20, 0.95)" : "rgba(245, 245, 245, 0.98)";
  ...
  header.style.cssText = `
    font-size: 14px;
    font-weight: 600;
    font-family: ${geminiTheme.light.font};
  `;
  ...
  const actionColors = currentTheme === "dark" ? geminiTheme.dark : geminiTheme.light;
  replaceBtn.style.background = actionColors.buttonBg;
  replaceBtn.style.color = actionColors.text;
  replaceBtn.style.border = `1px solid ${actionColors.border}`;
...
function createBubbleSaveIcon(promptText: string): HTMLButtonElement {
  ...
  // Match Gemini's button styling - hidden until hover
  btn.style.cssText = `
    display: inline-flex;
    align-items: center;
    justify-content: center;
...
function injectBubbleSaveIcons() {
  // Find all copy buttons with content_copy icon - these are associated with messages
  const copyIcons = document.querySelectorAll('mat-icon[fonticon="content_copy"], mat-icon[data-mat-icon-name="content_copy"]');

  copyIcons.forEach((copyIcon) => {
    // Get the button containing this copy icon
    const copyButton = copyIcon.closest('button') as HTMLElement | null;
    ...
    // Find the associated message container
    // Walk up to find a container that has query-text-line elements (user prompts)
    let container: HTMLElement | null = copyButton.parentElement;
    let promptText = '';
    let messageRow: HTMLElement | null = null;
    let isUserMessage = false;

    // Search up the DOM tree to find the prompt text
    let depth = 0;
    while (container && depth < 15) {
      // Check if this is a user message by looking for query-text-line elements
      // User messages have <p class="query-text-line"> elements
      const queryLines = container.querySelectorAll('p.query-text-line');
      if (queryLines.length > 0) {
        const lines = Array.from(queryLines)
      ...
      // Check if this is an AI response (has model-response or response-container)
      // AI responses don't have query-text-line, they have different structure
      if (container.querySelector('.model-response-text, message-content, .response-container')) {
        // This is an AI response, skip it
        return;
      }
    ...
    if (!isUserMessage || !promptText || promptText.length < 2) {
      return;
    }

    // Skip if already processed
    if (processedUserBubbles.has(copyButton)) return;
    processedUserBubbles.add(copyButton);

    // Create save icon
    const saveIcon = createBubbleSaveIcon(promptText);

    // Insert the save icon before the copy button
    ...
    // Also add to messageRow if found
    if (messageRow) {
      messageRow.addEventListener('mouseenter', showButton);
      messageRow.addEventListener('mouseleave', hideButton);
    }
    ...
    (saveIcon as any).__cleanup = () => {
      ...
      if (messageRow) {
        messageRow.removeEventListener('mouseenter', showButton);
        messageRow.removeEventListener('mouseleave', hideButton);
      }
    };
  });
}
...
async function showContextMenu(x: number, y: number) {
  ...
  // Fetch prompts grouped by pack for Gemini
  const groups = await listPromptsBySourceGrouped("gemini");
