...
// Content script for Perplexity AI - Pro users only
...
/**
 * Find and inject save buttons next to user message bubbles in Perplexity
 * Places button to the RIGHT of the user input bubble
 */
function injectBubbleSaveIcons() {
  // Find user query containers - elements with group/query class (can be div or h1)
  const queryContainers = document.querySelectorAll('[class*="group/query"]');

  queryContainers.forEach((queryContainer) => {
    if (!(queryContainer instanceof HTMLElement)) return;

    // Skip if already processed
    if (queryContainer.querySelector('.pp-bubble-save-btn')) return;

    // Find the bubble div inside (contains the user text)
    const bubbleDiv = queryContainer.querySelector('div.bg-offset') as HTMLElement;
    if (!bubbleDiv) return;

    // Get the prompt text from the span inside
    const textSpan = bubbleDiv.querySelector('span.select-text');
    const promptText = textSpan?.textContent?.trim();
    if (!promptText || promptText.length < 1) return;

    // Create a wrapper to hold the bubble and save button inline
    const wrapper = document.createElement("div");
    wrapper.className = "pp-bubble-wrapper";
    ...
    // Wrap the bubble and insert save button next to it
    bubbleDiv.parentNode?.insertBefore(wrapper, bubbleDiv);
    wrapper.appendChild(bubbleDiv);
    wrapper.appendChild(saveBtn);

    processedUserBubbles.add(queryContainer);

    // Show/hide on hover of the wrapper
...
async function showContextMenu(x: number, y: number) {
  ...
  const groups = await listPromptsBySourceGrouped("perplexity");
