...
// Use shared PromptPack brand theme
const bubbleTheme = promptPackTheme;
const ACCENT_COLOR = promptPackTheme.accent;
...
function findComposer(): ComposerEl | null {
  // Kimi uses a Lexical editor with contenteditable div
  // Primary: look for Kimi's specific chat input editor
  const kimiEditor = document.querySelector<HTMLElement>('.chat-input-editor[data-lexical-editor="true"]');
  if (kimiEditor && isVisible(kimiEditor)) {
    return kimiEditor;
  }

  // Fallback: any contenteditable with lexical
  const lexicalEditors = Array.from(document.querySelectorAll<HTMLElement>('[data-lexical-editor="true"][contenteditable="true"]'))
    .filter(isVisible);
  if (lexicalEditors.length) {
    lexicalEditors.sort((a, b) => b.clientWidth * b.clientHeight - a.clientWidth * a.clientHeight);
    return lexicalEditors[0];
  }
...
function tick() {
  ...
  // Always inject save icons next to user message bubbles
...
/**
 * Find and inject save buttons next to user message bubbles in Kimi
 * Places PromptPack logo to the right of .segment-content-box using absolute positioning
 */
function injectBubbleSaveIcons() {
  // Kimi user messages have structure:
  // <div class="segment-content">
  //   <div class="segment-content-box">
  //     <div class="user-content">user text</div>
  //   </div>
  // </div>
  // We place the save icon absolutely positioned to the right of segment-content-box

  const userContentElements = document.querySelectorAll('.user-content');

  userContentElements.forEach((userContent) => {
    if (!(userContent instanceof HTMLElement)) return;

    // Skip if already processed
    if (userContent.dataset.ppProcessed === "true") return;

    const promptText = userContent.textContent?.trim();
    if (!promptText || promptText.length < 1) return;

    // Mark as processed
    userContent.dataset.ppProcessed = "true";

    // Get the segment-content-box container
    const contentBox = userContent.closest('.segment-content-box');
    if (!contentBox || !(contentBox instanceof HTMLElement)) return;

    // Make content box position relative for absolute positioning
    const boxStyle = window.getComputedStyle(contentBox);
    if (boxStyle.position === "static") {
      contentBox.style.position = "relative";
    }

    // Create save button with PromptPack logo
    const saveBtn = document.createElement("button");
    saveBtn.className = "pp-bubble-save-btn";
    saveBtn.type = "button";
    saveBtn.setAttribute("aria-label", "Save to PromptPack");
    saveBtn.title = "Save to PromptPack";
    (saveBtn as any).__promptText = promptText;

    // Use inline SVG for the PromptPack logo (bookmark/save icon in brand purple)
    saveBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${ACCENT_COLOR}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    // Append save button to content box
    contentBox.appendChild(saveBtn);

    processedUserBubbles.add(userContent);

    // Find the segment-content container for hover
    const segmentContent = contentBox.closest('.segment-content');
    const hoverTarget = segmentContent || contentBox;
...
async function showContextMenu(x: number, y: number) {
  ...
  const groups = await listPromptsBySourceGrouped("kimi");
  ...
  if (totalPrompts === 0) {
    contextMenu.innerHTML = `
      <div style="padding: 12px 16px; color: ${colors.text}; text-align: center;">
    ...
    contextMenu.innerHTML = `
      <div style="padding: 8px 12px; font-weight: 600; border-bottom: 1px solid ${colors.border}; display: flex; align-items: center; gap: 8px; color: ${colors.text};">
...
function showTemplateInputDialog(
  ...
  header.style.cssText = `font-size: 14px; font-weight: 600; margin-bottom: 12px; color: ${ACCENT_COLOR};`;
  ...
  variables.forEach((varName) => {
    ...
    label.style.cssText = `display: block; font-size: 12px; color: ${bubbleTheme.secondary}; margin-bottom: 4px;`;
    ...
    input.style.cssText = `
      ...
      outline: none;
      background: ${bubbleTheme.inputBg};
